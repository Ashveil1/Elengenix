"""elengenix/agents/enricher.py — sub-agent of Adviser that gathers supplementary context.
...
into Elengenix. The Enricher is a *limited* agent
...
:class:`~elengenix.agent.crew.adviser.Adviser` and NEVER directly by the
...
:func:`elengenix.agent.crew.base.perform_agent_chain` loop via a small
``_EnricherToolExecutor`` adapter that implements the ``ToolExecutor``
protocol — exactly the pattern used by
:class:`elengenix.agent.crew.primary_agent.PrimaryAgent` and
:class:`elengenix.agent.crew.memorist.Memorist`.
"""
...
logger = logging.getLogger("elengenix.agent.crew.enricher")
...
class _EnricherToolExecutor:
    ...
    Implements the :class:`elengenix.agent.crew.base.ToolExecutor` protocol so
...
class Enricher:
    ...
    spawned by the :class:`~elengenix.agent.crew.adviser.Adviser` as a sub-chain.
    """
