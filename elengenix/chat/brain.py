"""elengenix/chat/brain.py — ElengenixAgent (interactive chat brain).

Relocated from the deprecated root-level ``core/brain.py`` during the
architecture consolidation. Powers ``cli/textual.py``,
``cli/interactive.py`` and ``integrations/bot.py``.
"""
...
import logging
import re
import sqlite3  # noqa: F401  (kept for parity with previous module surface)
import time
...
def _get_db_path() -> str:
    ...
    if _SQLITE_DB is None:
        ...
        _SQLITE_DB = str(Path.home() / ".elengenix" / "conversations.db")
    return _SQLITE_DB
...
def _analyze_intent(client: Any, query: str) -> str:
    """Lightweight intent classifier — delegates to elengenix.scanning."""
    try:
        from elengenix.scanning.universal import analyze_intent
...
class ElengenixAgent:
    """Interactive chat agent used by the TUI / CLI / Telegram bot."""
    ...
    # ------------------------------------------------------------------
    def __new__(cls, *args: Any, **kwargs: Any) -> "ElengenixAgent":
    ...
    def smart_orchestrator(self) -> Any:
        if self._smart_orchestrator is None:
            from elengenix.chat.smart_orchestrator import SmartOrchestrator
    ...
    def process_query(
        ...
        if intent in ("casual", "research", "security_chat") and not loop_target:
            ...
            chat_prompt = (
                "You are Elengenix AI v3.0, an expert security assistant "
    ...
    def _fingerprint_target_for_planning(self, target: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            from elengenix.scanning.planner import TargetFingerprinter
