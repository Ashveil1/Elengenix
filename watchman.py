"""
watchman.py — Elengenix 24/7 Target Monitoring Daemon
...
async def start_watching(target_list_path: str = "targets_to_watch.txt", interval_hours: int = 1):
    ...
    def shutdown_handler(sig, frame):
        logger.info("[STOP] Watchman shutting down. Finalizing state...")
        save_state(state)
        sys.exit(0)
    ...
    while True:
        try:
            ...
            for target in targets:
                logger.info(f"[SCAN] Monitoring Target: {target}")
                ...
                try:
                    ...
                    if current_hash == last_hash:
                        logger.info(f"[OK] No changes detected for {target}. Token usage optimized.")
                        state[target] = {"last_hash": current_hash, "last_check": datetime.now().isoformat()}
                        save_state(state)
                        continue

                    logger.info(f"[ALERT] New data detected for {target}. Invoking AI analysis...")
                except Exception as e:
            ...
            logger.info(f"[WAIT] Loop finished. Sleeping for {interval_hours} hour(s).")
            await asyncio.sleep(interval_seconds)
