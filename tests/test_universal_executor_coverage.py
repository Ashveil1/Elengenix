...
class TestExecuteActionCoverage:
    def test_run_tool_single(self):
        ...
        with patch("tools.tool_registry.registry") as mock_reg:
            ...
            result = ue.execute_action({
                "type": "run_tool",
                "params": {"tool": "test_tool", "target": "example.com"}
            })
            assert result.success
    ...
    def test_run_tool_not_available(self):
        ...
        with patch("tools.tool_registry.registry") as mock_reg:
            ...
            result = ue.execute_action({
                "type": "run_tool",
                "params": {"tool": "nonexistent_xyz", "target": "example.com"}
            })
            assert not result.success
    ...
    def test_run_tool_parallel(self):
        ...
        async def mock_execute(*args, **kwargs):
            return MagicMock(success=True, output="done", findings=[{"type": "xss"}], error_message="")
    ...
    def test_github_search(self):
        ue = UniversalExecutor()
        with patch("tools.github_intel.search_code", return_value=[
            {"repo": "test/repo", "file": "test.py", "content": "secret_key = 'xxx'"}
        ]):
            result = ue.execute_action({
                "type": "github_search",
                "params": {"query": "secret_key"}
            })
            assert result.success
    ...
    def test_js_analyze(self):
        ue = UniversalExecutor()
        with patch("tools.js_analyzer.analyze_js", return_value={
            "secrets": [{"type": "api_key", "value": "sk_test_123"}],
            "endpoints": ["/api/v1/users"]
        }):
            result = ue.execute_action({
                "type": "js_analyze",
                "params": {"url": "https://example.com/app.js"}
            })
            assert result.success
    ...
    def test_check_takeover(self):
        ue = UniversalExecutor()
        with patch("tools.subdomain_takeover.check_single_subdomain", return_value={
            "vulnerable": True, "service": "github-pages"
        }):
            result = ue.execute_action({
                "type": "check_takeover",
                "params": {"subdomain": "test.example.com"}
            })
            assert result.success
    ...
    def test_check_takeover_exception(self):
        ue = UniversalExecutor()
        with patch("tools.subdomain_takeover.check_single_subdomain", side_effect=Exception("dns error")):
            result = ue.execute_action({
                "type": "check_takeover",
                "params": {"subdomain": "test.example.com"}
            })
    ...
    def test_submit_findings_no_findings(self):
        ...
        result = ue.execute_action({
            ...
            "params": {"findings": [], "target": "example.com"}
        })
        assert not result.success
    ...
    def test_submit_findings(self):
        ...
        with patch("tools.vector_memory.remember"):
            result = ue.execute_action({
                ...
                "params": {
                    "findings": [
                        {"type": "xss", "endpoint": "/api", "severity": "high", "description": "Reflected XSS"}
                    ],
                    "target": "example.com"
                }
            })
    ...
    def test_web_search(self):
        ue = UniversalExecutor()
        with patch("tools.research_tool.search_web", return_value=[
