...
class TestKnowledgeGraph:
    ...
    def test_get_findings_for_asset(self):
        ...
        findings = [
            e.target for e in kg.edges if e.source == "a.com" and e.edge_type == EdgeType.HAS
        ]
...
class TestExploitChainBuilder:
    ...
    def test_attack_graph_add_edge(self):
        ...
        g.add_node(
            AttackNode("n2", NodeType.DATA_ACCESS, "Data", "desc", "Critical", "t", "x.com", 0.9)
        )
...
class TestObjectIDPermuter:
    ...
    def test_permutation_result_dataclass(self):
        ...
        pr = PermutationResult(
            url_a="https://x.com/api/users/100",
            url_b="https://x.com/api/users/200",
            status_a=200,
            status_b=403,
            len_a=1024,
            len_b=256,
            signal="idor_suspect",
            notes="Different responses",
        )
...
class TestWorkflowFuzzer:
    ...
    def test_workflow_plan_dataclass(self):
        ...
        wp = WorkflowPlan(
            plan_id="plan-1",
            title="Coupon Abuse",
            description="Test coupon flow",
            steps=[WorkflowStep("s1", "GET", "/api/coupons")],
            risk="medium",
        )
    ...
    def test_workflow_result_dataclass(self):
        ...
        wr = WorkflowResult(
            success=True,
            observations=[{"step": "s1", "status": 200}],
            anomalies=[],
            notes=["All good"],
        )
    ...
    def test_format_workflow_plans(self):
        ...
        plans = [
            WorkflowPlan(
                ...
                title="Test Plan",
                description="desc",
                steps=[WorkflowStep("s1", "GET", "/test")],
                risk="low",
            )
        ]
...
class TestMultimodalAgent:
    ...
    def test_vision_finding_dataclass(self):
        ...
        vf = VisionFinding(
            mode=VisionMode.TOKEN,
            text="Found API key",
            tokens=["AKIA1234567890123456"],
            confidence=0.9,
        )
...
class TestUserPreferences:
    ...
    def test_user_preferences_custom(self):
        ...
        up = UserPreferences(
            user_id=1, notifications_enabled=False, favorite_targets=["example.com"], theme="dark"
        )
...
class TestAgentDataclasses:
    ...
    def test_agent_thought_dataclass(self):
        ...
        thought = AgentThought(
            ...
            reasoning="Need to enumerate",
            action_taken="run subfinder",
            result="found 5 subs",
            confidence=0.8,
        )
...
class TestGovernance:
    def test_gate_decision_dataclass(self):
        ...
        gd = GateDecision(
            allowed=True, risk_level="SAFE", decision="approve", rationale="Test command"
        )
    ...
    def test_governance_gate_safe(self):
        ...
        g = Governance()
        decision = g.gate(
            "test-mission", "http://example.com", {"command": "curl http://example.com"}
        )
...
class TestUniversalAIClient:
    ...
    def test_ai_response_dataclass(self):
        ...
        ar = AIResponse(
            content="test", model="gpt-4", usage={"prompt_tokens": 10, "completion_tokens": 5}
        )
...
class TestUniversalExecutor:
    def test_execution_result_dataclass(self):
        ...
        er = ExecutionResult(
            success=True, output="completed", error="", action_type="shell", metadata={}
        )
...
class TestLogicFlawEngine:
    ...
    def test_logic_flaw_dataclass(self):
        ...
        flaw = LogicFlaw(
            flaw_type="double_spend",
            endpoint="/api/pay",
            description="Can spend twice",
            evidence="Double spend occurred",
            severity="high",
            confidence=0.8,
            remediation="Use idempotency keys",
        )
...
class TestPayloadMutation:
    ...
    def test_injection_context_dataclass(self):
        ...
        ic = InjectionContext(
            category="sqli", sinks=["id"], quote_style="single", transport="http", extra_filters=[]
        )
...
class TestExploitation:
    def test_exploit_proof_dataclass(self):
        ...
        ep = ExploitProof(
            ...
            steps=["Step 1: Inject payload", "Step 2: Trigger alert"],
        )
        assert ep.title == "XSS"
...
class TestHuntEngine:
    ...
    def test_hunt_finding_dataclass(self):
        ...
        hf = HuntFinding(
            ...
            severity="Medium",
            title="Open Redirect",
            evidence="Location header reflects",
        )
...
class TestVectorMemory:
    def test_memory_entry_dataclass(self):
        ...
        me = MemoryEntry(
            id="1",
            content="Found XSS vulnerability",
            target="example.com",
...
class TestProtocolAnalyzer:
    ...
    def test_protocol_finding_dataclass(self):
        ...
        pf = ProtocolFinding(
            ...
            severity="high",
            confidence=0.8,
            description="No auth on MQTT broker",
        )
...
class TestLogicAnalyzer:
    def test_logic_hypothesis_dataclass(self):
        ...
        lh = LogicHypothesis(
            hyp_id="LH-1",
            title="Price can be manipulated",
            description="Price manipulation in checkout",
            confidence=0.7,
            tags=["price"],
            suggested_tests=["test price param"],
        )
...
class TestContextCompressor:
    def test_compression_result_dataclass(self):
        ...
        cr = CompressionResult(
            ...
            summary="Key findings preserved",
        )
...
class TestBOLATester:
    def test_session_dataclass(self):
        ...
        s = Session(
            name="admin", headers={"Authorization": "Bearer token123"}, cookies={"session": "abc"}
        )
        assert s.name == "admin"
...
class TestWAFDetector:
    def test_waf_signature_dataclass(self):
        ...
        ws = WAFSignature(
            name="Cloudflare",
            block_status=403,
            body_pattern="cloudflare",
            header_patterns=["cf-ray"],
        )
...
class TestNativeScanner:
    def test_scan_target_dataclass(self):
        ...
        st = ScanTarget(
            url="https://example.com", method="GET", headers={}, body=None, timeout=10.0
        )
...
class TestEndpointDiscovery:
    def test_endpoint_dataclass(self):
        ...
        ep = Endpoint(
            url="https://example.com/api/users", method="GET", params=["id"], source="manual"
        )
...
class TestPDFReportGenerator:
    ...
    def test_report_metadata_dataclass(self):
        ...
        rm = ReportMetadata(
            title="Security Report", target="example.com", author="Security Team", date="2025-01-01"
        )
...
class TestProgressDisplay:
    def test_scan_phase_dataclass(self):
        ...
        sp = ScanPhase(
            id="1", name="recon", subtasks=["dns", "port_scan"], status="running", progress=50.0
        )
...
class TestMarketplace:
    def test_plugin_entry_dataclass(self):
        ...
        pe = PluginEntry(
            name="test_plugin", version="1.0.0", description="A test plugin", author="tester"
        )
...
class TestWordlistManager:
    def test_path_suggestion_dataclass(self):
        ...
        ps = PathSuggestion(
            path="/api/admin",
            confidence=0.8,
            reasoning="Admin endpoint likely",
    ...
    def test_wordlist_config_dataclass(self):
        ...
        wc = WordlistConfig(
            category="admin", custom_paths=["/admin", "/login"], tech_stack=["php", "mysql"]
        )
...
class TestAuthTester:
    def test_auth_finding_dataclass(self):
        ...
        af = AuthFinding(
            ...
            severity="high",
            description="Using alg=none",
            evidence="JWT decoded with alg=none",
            remediation="Reject none algorithm",
        )
...
class TestExploitChainAdditional:
    ...
    def test_attack_graph_find_paths_no_path(self):
        ...
        g.add_node(
            AttackNode(
                "n2", NodeType.DATA_ACCESS, "Data", "desc", "Critical", "t", "other.com", 0.9
            )
        )
        paths = g.find_paths("n1", [NodeType.DATA_ACCESS])
        assert paths == []
...
class TestCoverageAnalyzerTempDB:
    def test_record_endpoint_and_report(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            ca.record_endpoint(
                url="https://example.com/api/users", method="GET", params=["id", "name"]
            )
            ca.record_endpoint(
                url="https://example.com/api/orders", method="POST", params=["order_id"]
            )
    ...
    def test_signal_strength_strong(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            s = f._signal_strength(
                {
                    "cvss": 9.8,
                    "evidence": "multiple lines of evidence",
                    "severity": "Critical",
                    "reproducible": True,
                }
            )
