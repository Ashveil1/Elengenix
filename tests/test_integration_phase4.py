...
def test_m11_full_pipeline_e2e(target_server):
    ...
    fuzz_results = fuzzer.fuzz_parameter(
        f"{target_server}/api/reflect", "q", evasion_payloads
    )
    # Record tests
    for fr in fuzz_results:
        coverage.record_test(
            url=f"{target_server}/api/reflect", method="GET",
            tool="active_fuzzer", injection_point="param:q",
            payload=fr.payload, status=fr.status,
    ...
    bola_results = bola.test_endpoint_collection(
        f"{target_server}/api/users/{{id}}", ["1", "2", "3"],
    ...
    for fr in fuzz_results:
        learning.remember(ExploitRecord(
            target="test", tech_stack=["javascript"],
            vuln_class="xss", tool="active_fuzzer",
            payload=fr.payload, success=fr.is_interesting,
            confidence=fr.score,
        ))
    ranked = learning.rank_tools(vuln_class="xss")
    assert len(ranked) >= 1
    print(f"  [M7] Top tool for xss: {ranked[0][0]} (rate={ranked[0][1]:.2f}, samples={ranked[0][2]})")
    ...
    print(f"  [M6] Coverage: {report.coverage_pct:.1f}% ({report.tested_param_slots}/{report.total_param_slots} params)")
    ...
    print(f"[OK] M6 CoverageAnalyzer: {report.total_endpoints} endpoints, {report.coverage_pct:.1f}% coverage")
    print(f"[OK] M7 LearningEngine: ranked {len(ranked)} tools by success rate")
    print(f"[OK] M8 BOLATester: found {bola_count} BOLA")
    print(f"[OK] M9 SmartWAFDetector: detected {waf_result.waf_name}, {len(waf_result.suggested_evasions)} evasions")
    print("=" * 60)
...
def test_m11_real_workflow_simulation(target_server):
    ...
    fuzz_results = fuzzer.fuzz_parameter(
        f"{target_server}/api/reflect", "q", payloads
    )
    for fr in fuzz_results:
        coverage.record_test(
            url=f"{target_server}/api/reflect", method="GET",
    ...
    bola_results = bola.test_endpoint_collection(
        f"{target_server}/api/users/{{id}}", ["1", "2"],
        session_a="victim", session_b="attacker"
    )
    ...
    for fr in fuzz_results:
        learning.remember(ExploitRecord(
            target="sim_target", tech_stack=["javascript"],
            vuln_class="xss", tool="active_fuzzer",
            payload=fr.payload, success=fr.is_interesting,
        ))
    for br in bola_results:
        if br.is_bola:
            learning.remember(ExploitRecord(
                target="sim_target", tech_stack=["api"],
                vuln_class="bola", tool="bola_tester",
                payload=br.object_id, success=True, severity=br.severity,
            ))
    ...
    print(f"[WORKFLOW] fuzz={len(fuzz_results)}, bola={len(bola_results)}, "
