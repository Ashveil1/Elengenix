...
def test_hunt_engine_imports() -> None:
    ...
    from tools.hunt_engine import (
        HuntEngine, HuntReport, HuntFinding, HuntPhase,
        Severity, compute_risk_score, correlate_chains,
        report_to_console, report_to_dict, save_report,
    )
...
def test_risk_score_live_critical() -> None:
    ...
    fs = [
        HuntFinding(phase="zero_day", category="jwt", severity="Critical",
                   title="JWT alg=none accepted", url="http://target/api",
    ...
    fs.append(HuntFinding(phase="zero_day", category="jwt", severity="Critical",
                          title="JWT alg=none_no_typ accepted", url="http://target/api",
                          details="Server returned 200"))
    score, level = compute_risk_score(fs)
    assert score >= 50
...
def test_correlation_same_url() -> None:
    ...
    fs = [
        HuntFinding(phase="zero_day", category="jwt", severity="High",
                   title="JWT alg=none", url="http://x.com/api",
                   details="confirmed"),
        HuntFinding(phase="smart", category="bola", severity="High",
                   title="BOLA on /api", url="http://x.com/api",
                   details="confirmed"),
    ]
...
def test_correlation_ignores_static_candidates() -> None:
    ...
    fs = [
        ...
        HuntFinding(phase="zero_day", category="jwt", severity="Critical",
                   title="JWT forgery CANDIDATE (not tested): alg_none"),
        HuntFinding(phase="smart", category="bola", severity="High",
                   title="BOLA", url="http://x.com/api", details="confirmed"),
    ]
    chains = correlate_chains(fs)
...
def test_correlation_jwt_idor_chain() -> None:
    ...
    from tools.hunt_engine import HuntFinding, correlate_chains
    fs = [
        HuntFinding(phase="zero_day", category="jwt_confusion", severity="Critical",
                   title="JWT alg=none accepted", url="http://target/api",
                   details="Server returned 200 with forged token"),
        HuntFinding(phase="smart", category="bola", severity="High",
                   title="IDOR on /api/user", url="http://target/api/user",
...
def test_report_to_dict_serializable() -> None:
    ...
    from tools.hunt_engine import (
        HuntEngine, HuntReport, HuntFinding, HuntPhase, report_to_dict,
    )
...
def test_report_to_console_format() -> None:
    ...
    from tools.hunt_engine import (
        HuntEngine, HuntReport, HuntFinding, HuntPhase, report_to_console,
    )
    r = HuntReport(target="example.com", started_at="2026-01-01T00:00:00Z")
    r.findings = [
        HuntFinding(phase="zero_day", category="jwt", severity="Critical",
                   title="JWT alg=none accepted", url="http://target/api",
                   details="confirmed"),
        HuntFinding(phase="recon", category="baseline", severity="Informational",
                   title="Recon", url="http://target"),
    ]
    r.phases = [
...
def test_save_report_creates_files(tmp_path: Path) -> None:
    ...
    from tools.hunt_engine import (
        HuntReport, HuntFinding, save_report,
    )
    r = HuntReport(target="example.com", started_at="2026-01-01T00:00:00Z")
...
def test_hunt_engine_skip_phases() -> None:
    ...
    from tools.hunt_engine import HuntEngine
    engine = HuntEngine(target="httpbin.org",
                        skip_phases=["zero_day", "logic"],
                        quiet=True)
    report = engine.hunt_sync()
