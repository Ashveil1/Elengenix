...
def test_all_scanners_instantiate():
    ...
    # Test instantiation
    scanners = [
        SSRFScanner(), SSTIScanner(), XXEScanner(),
        DeserializationScanner(), GraphQLScanner(),
        RaceConditionTester(), APISchemaDiffer(),
        SupplyChainAnalyzer(), LogicFlawEngine(),
        CORSChecker(), JWTTester(),
    ]
...
def test_all_agent_modules_import():
    ...
    from agents.agent_helpers import _safe_operation, _extract_target_from_text
