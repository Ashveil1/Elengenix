...
class TestVisualizations:
    ...
    def test_vulnerability_heatmap_custom_colors(self):
        ...
        heat = VulnerabilityHeatmap(endpoints=["a"], vuln_types=["X"], severity_colors=custom)
        heat.set("a", "X", "high", 1)
        result = heat.render()
        assert isinstance(result, Panel)
    ...
    def test_risk_gauge_custom_colors(self):
        ...
        gauge = RiskGauge(
            value=75, max_value=100, low_color="#00ff00", mid_color="#ffff00", high_color="#ff0000"
        )
        result = gauge.render()
        assert isinstance(result, Panel)
...
class TestWelcome:
    ...
    def test_build_welcome_renderable_custom(self):
        ...
        result = build_welcome_renderable(
            mission=mission, activity=activity, theme_name="CYBERPUNK"
        )
