...
def test_logic_finding_id_autogenerated() -> None:
    ...
    from tools.logic_flaw_engine import LogicFinding, DetectorCategory, Severity
    f = LogicFinding(
        title="X",
        target="t",
        endpoint="/x",
...
def test_logic_finding_to_dict_serializable() -> None:
    ...
    f = LogicFinding(title="t", target="t", endpoint="/t", description="d",
...
def test_helper_keyword_predicates() -> None:
    ...
    assert is_price_endpoint({"url": "/api/pay", "method": "POST",
                              "body": {"amount": 1}})
...
def test_price_detector_flags_quantity_zero() -> None:
    ...
    engine = LogicFlawEngine()
    endpoints = [
        {"url": "https://api.com/place_order", "method": "POST",
...
def test_price_detector_currency_confusion() -> None:
    ...
    engine = LogicFlawEngine()
    endpoints = [
        {"url": "https://api.com/charge", "method": "POST",
...
def test_price_detector_discount_stacking() -> None:
    ...
    engine = LogicFlawEngine()
    endpoints = [
        {"url": "https://api.com/apply_coupon", "method": "POST",
...
def test_price_detector_integer_overflow_payloads() -> None:
    ...
    from tools.logic_flaw_engine import PriceManipulationDetector, LogicFlawConfig
    det = PriceManipulationDetector(LogicFlawConfig())
    endpoints = [
        {"url": "https://api.com/pay", "method": "POST",
         "body": {"amount": 100}},
    ]
...
def test_race_detector_static_heuristic() -> None:
    ...
    endpoints = [
        {"url": "https://api.com/apply_coupon", "method": "POST",
         "body": {"coupon": "WELCOME"}},
        {"url": "https://api.com/withdraw", "method": "POST",
         "body": {"amount": 100, "user_id": 1}},
    ]
...
def test_race_detector_dynamic_burst(mock_server: str) -> None:
    ...
    from tools.logic_flaw_engine import (
        LogicFlawEngine, LogicFlawConfig, RaceConditionDetector,
    )
    ...
    endpoints = [
        {"url": f"{mock_server}/api/withdraw", "method": "POST",
...
def test_state_machine_skips_step_via_query() -> None:
    ...
    engine = LogicFlawEngine()
    endpoints = [
        {"url": "https://api.com/onboard", "method": "POST",
...
def test_auth_reset_token_flagged() -> None:
    ...
    engine = LogicFlawEngine()
    endpoints = [
        {"url": "https://api.com/auth/forgot_password", "method": "POST",
         "body": {"email": "a@b.c"}},
    ]
...
def test_auth_2fa_bypass_flagged() -> None:
    ...
    engine = LogicFlawEngine()
    endpoints = [
        {"url": "https://api.com/auth/verify_2fa", "method": "POST",
         "body": {"code": "123456"}},
    ]
    det = engine.detectors[3]
    findings = asyncio.run(det.detect("api.com", endpoints))
...
def test_auth_session_fixation_flagged() -> None:
    ...
    engine = LogicFlawEngine()
    endpoints = [
        {"url": "https://api.com/auth/login", "method": "POST",
...
def test_auth_oauth_external_redirect_flagged() -> None:
    ...
    engine = LogicFlawEngine()
    endpoints = [
        {"url": "https://api.com/oauth/authorize", "method": "GET",
    ...
    assert any("evil.com" in f.title for f in findings), \
...
def test_auth_remember_me_flagged() -> None:
    ...
    engine = LogicFlawEngine()
    endpoints = [
        {"url": "https://api.com/auth/remember_me", "method": "POST",
         "body": {"remember": "true"}},
    ]
    det = engine.detectors[3]
    findings = asyncio.run(det.detect("api.com", endpoints))
...
def test_authorization_role_param_flagged() -> None:
    ...
    engine = LogicFlawEngine()
    endpoints = [
        {"url": "https://api.com/api/users/list", "method": "GET",
         "params": {"role": "user"}},
    ]
    det = engine.detectors[4]
    findings = asyncio.run(det.detect("api.com", endpoints))
...
def test_workflow_missing_idempotency_key() -> None:
    ...
    engine = LogicFlawEngine()
    endpoints = [
        {"url": "https://api.com/api/pay", "method": "POST",
         "body": {"amount": 100, "user_id": 1}},
    ]
...
def test_workflow_force_param_flagged() -> None:
    ...
    engine = LogicFlawEngine()
    endpoints = [
        {"url": "https://api.com/api/refund", "method": "POST",
...
def test_constraint_time_param_flagged() -> None:
    ...
    engine = LogicFlawEngine()
    endpoints = [
        {"url": "https://api.com/api/charge", "method": "POST",
...
def test_constraint_geo_param_flagged() -> None:
    ...
    engine = LogicFlawEngine()
    endpoints = [
        {"url": "https://api.com/api/content", "method": "GET",
         "params": {"country": "US"}},
    ]
    det = engine.detectors[6]
    findings = asyncio.run(det.detect("api.com", endpoints))
...
def test_constraint_kyc_param_flagged() -> None:
    ...
    engine = LogicFlawEngine()
    endpoints = [
        {"url": "https://api.com/api/account/tier", "method": "POST",
...
def test_constraint_trial_param_flagged() -> None:
    ...
    engine = LogicFlawEngine()
    endpoints = [
        {"url": "https://api.com/api/signup", "method": "POST",
...
def test_constraint_min_max_amount_flagged() -> None:
    ...
    engine = LogicFlawEngine()
    endpoints = [
        {"url": "https://api.com/api/transfer", "method": "POST",
         "body": {"amount": 100, "user_id": 1}},
    ]
    det = engine.detectors[6]
    findings = asyncio.run(det.detect("api.com", endpoints))
...
def test_inference_engine_novelty_unique() -> None:
    from tools.logic_flaw_engine import (
        InferenceEngine, LogicFlawConfig, LogicFinding, DetectorCategory, Severity,
    )
...
def test_inference_engine_impact_higher_for_critical_cats() -> None:
    ...
    ie = InferenceEngine(LogicFlawConfig())
    auth_f = LogicFinding(title="t", target="t", endpoint="/a", category=DetectorCategory.AUTH_LOGIC)
    constraint_f = LogicFinding(title="t", target="t", endpoint="/a", category=DetectorCategory.BUSINESS_CONSTRAINT)
    ...
    print(f"[OK] inference: auth_logic={auth_f.impact_score}, constraint={constraint_f.impact_score}")


def test_inference_engine_reproducibility_static_vs_dynamic() -> None:
    from tools.logic_flaw_engine import (
        InferenceEngine, LogicFlawConfig, LogicFinding, DetectorCategory, Evidence,
    )
    ie = InferenceEngine(LogicFlawConfig())
    f_static = LogicFinding(title="t", target="t", endpoint="/a", category=DetectorCategory.AUTH_LOGIC)
    f_dynamic = LogicFinding(title="t", target="t", endpoint="/a", category=DetectorCategory.AUTH_LOGIC)
...
def test_inference_engine_risk_recomputed() -> None:
    ...
    ie = InferenceEngine(LogicFlawConfig())
    f = LogicFinding(title="t", target="t", endpoint="/a",
...
def test_correlation_emits_chains() -> None:
    from tools.logic_flaw_engine import (
        CorrelationEngine, LogicFinding, DetectorCategory, Severity,
    )
    engine = CorrelationEngine()
    f1 = LogicFinding(
        title="race", target="t", endpoint="/a",
        category=DetectorCategory.RACE_CONDITION, severity=Severity.CRITICAL,
        tags=["race", "toctou"],
    )
    f2 = LogicFinding(
        title="bola", target="t", endpoint="/b",
        category=DetectorCategory.AUTHORIZATION, severity=Severity.HIGH,
        tags=["bola", "idor"],
    )
...
def test_correlation_currency_negative_amount() -> None:
    ...
    engine = CorrelationEngine()
    f1 = LogicFinding(
        title="currency", target="t", endpoint="/pay",
        category=DetectorCategory.PRICE_MANIPULATION, severity=Severity.CRITICAL,
        tags=["currency_confusion", "decimal_mismatch"],
    )
    f2 = LogicFinding(
        title="negative", target="t", endpoint="/refund",
        category=DetectorCategory.PRICE_MANIPULATION, severity=Severity.HIGH,
...
def test_engine_http_integration_with_mock(mock_server: str) -> None:
    ...
    from tools.logic_flaw_engine import LogicFlawEngine, LogicFlawConfig
    cfg = LogicFlawConfig(race_default_concurrency=4, race_default_attempts=2,
...
def test_end_to_end_mock_vulnerable_app(mock_server: str) -> None:
    ...
    from tools.logic_flaw_engine import LogicFlawEngine, LogicFlawConfig
    cfg = LogicFlawConfig(http_timeout_seconds=3.0,
                          race_default_concurrency=4, race_default_attempts=2)
    engine = LogicFlawEngine(cfg)
    endpoints = [
        ...
        {"url": f"{mock_server}/api/transfer", "method": "POST",
         "body": {"amount": 100, "user_id": 1}},
        {"url": f"{mock_server}/api/refund", "method": "POST",
         "body": {"amount": 50, "user_id": 1}},
        {"url": f"{mock_server}/api/place_order", "method": "POST",
         "body": {"quantity": 1, "product_id": 1}},
        # Discount stacking / race
        {"url": f"{mock_server}/api/apply_coupon", "method": "POST",
         "body": {"coupon": "WELCOME"}},
        # Auth logic
        {"url": f"{mock_server}/api/login", "method": "POST",
         "body": {"username": "u", "password": "p"}},
        {"url": f"{mock_server}/api/verify_2fa", "method": "POST",
         "body": {"code": "123456"}},
    ]
    findings = asyncio.run(engine.analyze("api.com", endpoints))
    ...
    print(f"[OK] end-to-end: {len(findings)} findings, top={findings[0].title}, "
