...
def test_remember_batch(engine):
    ...
    from tools.learning_engine import ExploitRecord
    records = [
        ExploitRecord(target=f"https://t{i}.com", tech_stack=["php"], vuln_class="xss",
                      tool="dalfox", payload="<script>", success=(i % 2 == 0), confidence=0.8)
        for i in range(5)
    ]
...
def test_recall_similar_finds_matching(engine):
    ...
    from tools.learning_engine import ExploitRecord
    engine.remember(ExploitRecord(
        target="https://php-target.com", tech_stack=["php", "mysql", "wordpress"],
        vuln_class="sqli", tool="sqlmap", payload="' OR 1=1--", success=True, confidence=0.9
    ))
    engine.remember(ExploitRecord(
        target="https://java-target.com", tech_stack=["java", "oracle"],
        vuln_class="sqli", tool="sqlmap", payload="' UNION SELECT NULL--", success=True, confidence=0.85
    ))
    ...
    print(f"[RECALL] for [php,mysql]: top tool={similar[0].tool}, payload={similar[0].payload[:30]}")
...
def test_rank_tools_by_success_rate(engine):
    ...
    for _ in range(3):
        engine.remember(ExploitRecord(
            target="t.com", tech_stack=["php"], vuln_class="sqli",
            tool="sqlmap", payload="p", success=True, confidence=0.9
        ))
    # nuclei: 1/3 success
    for i in range(3):
        engine.remember(ExploitRecord(
            target="t.com", tech_stack=["php"], vuln_class="sqli",
            tool="nuclei", payload=f"p{i}", success=(i == 0), confidence=0.5
        ))
    # ffuf: 2/2 success
    for _ in range(2):
        engine.remember(ExploitRecord(
            target="t.com", tech_stack=["php"], vuln_class="sqli",
            tool="ffuf", payload="p", success=True, confidence=0.7
        ))
...
def test_suggest_payloads_returns_working_ones(engine):
    ...
    for _ in range(3):
        engine.remember(ExploitRecord(
            target="t.com", tech_stack=["php"], vuln_class="xss",
            tool="dalfox", payload="<script>alert(1)</script>", success=True, confidence=0.9
        ))
    # Store a different one once
    engine.remember(ExploitRecord(
        target="t.com", tech_stack=["php"], vuln_class="xss",
        tool="dalfox", payload="<svg onload=alert(1)>", success=True, confidence=0.7
    ))
...
def test_filter_by_min_success_rate(engine):
    ...
    for _ in range(2):
        engine.remember(ExploitRecord(
            target="t.com", tech_stack=["php"], vuln_class="sqli",
            tool="sqlmap", payload="p", success=True
        ))
    # 1/2 success for nuclei
    engine.remember(ExploitRecord(
        target="t.com", tech_stack=["php"], vuln_class="sqli",
        tool="nuclei", payload="p1", success=True
    ))
    engine.remember(ExploitRecord(
        target="t.com", tech_stack=["php"], vuln_class="sqli",
        tool="nuclei", payload="p2", success=False
    ))
...
def test_get_stats_groups_by_vuln_and_tool(engine):
    ...
    for _ in range(3):
        engine.remember(ExploitRecord(
            target="t.com", tech_stack=["php"], vuln_class="xss",
            tool="dalfox", payload="p", success=True
        ))
    for _ in range(2):
        engine.remember(ExploitRecord(
            target="t.com", tech_stack=["php"], vuln_class="sqli",
...
def test_chroma_integration_works(engine):
    ...
    if not chroma_engine._chroma_collection:
        return
    rec = ExploitRecord(
        target="https://chromadb-test.com", tech_stack=["python", "django"],
        vuln_class="ssrf", tool="ffuf", payload="file:///etc/passwd", success=True
    )
...
def test_persistence_across_instances(tmp_path, monkeypatch):
    ...
    e1.remember(ExploitRecord(
        target="t.com", tech_stack=["php"], vuln_class="xss",
        tool="dalfox", payload="<script>", success=True
    ))
...
def test_dual_layer_ranking(engine):
    ...
    for i in range(5):
        engine.remember(ExploitRecord(
            target=f"t{i}.com", tech_stack=["php", "mysql"], vuln_class="sqli",
            tool="sqlmap", payload=f"payload{i}", success=(i < 4), confidence=0.8 + i * 0.02
        ))
