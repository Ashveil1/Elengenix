...
class TestAutonomousAgentExecWafDetect:
    def test_waf_detected(self):
        with _patch_requests() as mock_req:
            ...
            with patch(
                "tools.waf_signatures.detect_waf_from_response", return_value=("Cloudflare", 0.9)
            ):
...
class TestAutonomousAgentExecHeaderAudit:
    def test_cors_wildcard(self):
        with _patch_requests() as mock_req:
            mock_req.options.return_value = _mock_response(
...
class TestAutonomousAgentExecRecon:
    ...
    def test_recon_success(self, MockEngine):
        ...
        mock_result.findings = [
            {"type": "sub", "severity": "info", "title": "sub", "target": "x", "description": "d"}
        ]
...
class TestAutonomousAgentExecCorsScan:
    def test_cors_scan_issues(self):
        ...
        tools.cors_checker.check_cors = lambda target: {
            "issues": [
                {"severity": "HIGH", "origin": "evil.com", "reason": "reflected", "headers": {}}
            ]
        }
        try:
...
class TestAutonomousAgentExecuteAction:
    ...
    def test_dispatch_unknown_action(self):
        ...
        assert (
            AutonomousAgent(ai_client=None)._execute_action(
                _make_action("nonexistent"), _make_state()
            )
            == []
        )
    ...
    def test_dispatch_threat_model(self):
        ...
        findings = AutonomousAgent(ai_client=None)._execute_action(
            _make_action("threat_model"), _make_state()
        )
        assert isinstance(findings, list)
...
class TestAutonomousAgentGraphql:
    def test_graphql_scan(self):
        ...
        # pyrefly: ignore [missing-attribute]
        tools.graphql_scanner.scan_graphql = lambda t, **kw: [
            {"title": "GraphQL", "severity": "info"}
        ]
        try:
...
class TestAutonomousAgentRaceCondition:
    def test_race_scan(self):
        ...
        # pyrefly: ignore [missing-attribute]
        tools.race_condition_tester.scan_race_conditions = lambda t, **kw: [
            {"title": "Race", "severity": "high"}
        ]
        try:
...
class TestPrototypePollutionDetector:
    ...
    def test_gadget_reflected(self):
        det, http = self._make_detector()
        http._async_return = _mock_async_resp(
            200, '{"polluted":"elenheur-1337","lodash.merge":true}'
        )
        findings = run_async(det.detect("https://example.com"))
    ...
    def test_analyze_response_mass_assignment(self):
        det, _ = self._make_detector()
        assert (
            det._analyze_response(
                "https://x.com",
                {"isAdmin": True},
                _mock_async_resp(200, '{"isadmin": true, "role": "admin"}'),
            )
            is not None
        )
    ...
    def test_build_stack_finding(self):
        det, _ = self._make_detector()
        f = det._build_stack_finding(
            "https://x.com", _mock_async_resp(500, "Object.prototype error")
        )
...
class TestRaceConditionDetector:
    ...
    def test_status_divergence(self):
        det, _ = self._make_detector()
        findings = det._analyze(
            "https://example.com",
            "POST",
            2,
            [_mock_async_resp(200, "ok"), _mock_async_resp(500, "err")],
        )
    ...
    def test_length_spread(self):
        ...
        findings = det._analyze(
            ...
            "POST",
            2,
            [_mock_async_resp(200, "x" * 100), _mock_async_resp(200, "x" * 10)],
        )
    ...
    def test_less_than_2_results(self):
        ...
        assert (
            det._analyze(
                ...
                "POST",
                12,
                [{"status": 200, "length": 100, "elapsed_ms": 10, "text": "ok"}],
            )
            == []
        )
...
class TestSSTIDetector:
    ...
    def test_jinja2_stack_trace(self):
        ...
        async def side_effect(method, url, **kwargs):
            call_count[0] += 1
            return (
                _mock_async_resp(500, "jinja2.exceptions.TemplateSyntaxError")
                if call_count[0] > 1
                else _mock_async_resp(200, "ok")
            )
    ...
    def test_analyze_reflection(self):
        det, _ = self._make_detector()
        findings = det._analyze(
            "https://x.com", "q", "{{7*7}}", "49", "normal page", "Result: 49", {"_method": "GET"}
        )
    ...
    def test_introspection_enabled(self):
        ...
        schema = {
            "types": [
                {
                    "name": "User",
                    "fields": [{"name": "password", "isDeprecated": False}],
                    "kind": "OBJECT",
                }
            ],
            ...
            "types": [
                {
                    "name": "User",
                    "fields": [
                        {"name": "password", "isDeprecated": False},
                        {"name": "token", "isDeprecated": True},
                    ],
                    "kind": "OBJECT",
                }
            ],
...
class TestJWTAlgorithmDetector:
    ...
    def test_forge_tokens(self):
        ...
        assert (
            "alg_none" in attacks
            and "kid_path_traversal" in attacks
            and "hs256_confusion" in attacks
        )
    ...
    def test_detect_on_endpoint_accepted(self):
        ...
        async def side_effect(method, url, **kwargs):
            call_count[0] += 1
            return (
                _mock_async_resp(401, "Unauthorized")
                if call_count[0] == 1
                else _mock_async_resp(200, "Welcome admin")
            )
...
class TestJWTHelpers:
    ...
    def test_is_jwt_valid(self):
        ...
        assert (
            not _is_jwt("not-a-jwt")
            and not _is_jwt("")
            and not _is_jwt(None)
            and not _is_jwt("a.b")
        )
...
class TestSmartAnomalyDetector:
    ...
    def test_anomaly_detected(self):
        ...
        async def side_effect(method, url, **kwargs):
            call_count[0] += 1
            return (
                _mock_async_resp(500, "Internal Server Error")
                if call_count[0] > 5
                else _mock_async_resp(200, "normal")
            )
    ...
    def test_header_anomaly(self):
        ...
        s1 = ResponseSnapshot(
            status=200, length=100, elapsed_ms=10, headers={"A": "1", "B": "2"}, text=""
        )
    ...
    def test_outlier_length(self):
        ...
        assert (
            det._outlier(
                ResponseSnapshot(status=200, length=500, elapsed_ms=10, headers={}, text="x"), stats
            )
            is not None
        )
    ...
    def test_outlier_5xx(self):
        ...
        assert (
            det._outlier(
                ResponseSnapshot(status=500, length=100, elapsed_ms=10, headers={}, text="x"), stats
            )
            is not None
        )
    ...
    def test_outlier_none(self):
        ...
        assert (
            det._outlier(
                ResponseSnapshot(status=200, length=100, elapsed_ms=10, headers={}, text="x"), stats
            )
            is None
        )
...
class TestResponseSnapshot:
    def test_from_response(self):
        ...
        snap = ResponseSnapshot.from_response(
            {
                "status": 200,
                "length": 100,
                "elapsed_ms": 5.0,
...
class TestFindingGraph:
    ...
    def test_chain_score(self):
        ...
        g.nodes = {
            "n1": FindingNode(
                node_id="n1", kind="finding", label="a", metadata={"severity": "high"}
            ),
            "n2": FindingNode(
                node_id="n2", kind="finding", label="b", metadata={"severity": "critical"}
            ),
        }
...
class TestZeroDayEngine:
    def test_build_detectors_all_enabled(self):
        ...
        assert all(
            k in engine._detectors for k in ["prototype", "mass_assignment", "ssti", "graphql"]
        )
        engine.close()
    ...
    def test_severity_rank(self):
        ...
        f = Finding(
            detector="t", title="t", severity=SeverityLevel.CRITICAL, vuln_class=VulnClass.ZERO_DAY
        )
...
class TestConfigWizardSaveEnvVar:
    ...
    def test_save_overwrite(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            lines = [
                l
                for l in wizard.env_file.read_text().splitlines()
                if l.startswith("TEST_KEY_OVER=")
            ]
...
class TestConfigWizardSelectModel:
    ...
    def test_select_model_skip(self, mock_console):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ConfigWizard(config_dir=Path(tmp))._select_model(
                AIProviderConfig(
                    name="TP",
                    env_key="TEST_PK_API_KEY",
...
class TestConfigWizardFetchRemoteModels:
    def test_fetch_models_success(self):
        ...
        mock_resp.json.return_value = {
            "data": [{"id": "model-a"}, {"id": "model-b"}, {"id": "embedding-model"}]
        }
        with _patch_requests() as mock_req:
            mock_req.get.return_value = mock_resp
            with tempfile.TemporaryDirectory() as tmp:
                models = ConfigWizard(config_dir=Path(tmp))._fetch_remote_models(
                    AIProviderConfig(
                        name="TP",
                        env_key="TF",
                        base_url="https://api.test.com/v1",
    ...
    def test_fetch_models_anthropic(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            assert (
                ConfigWizard(config_dir=Path(tmp))._fetch_remote_models(
                    AIProviderConfig(
                ...
                assert (
                    ConfigWizard(config_dir=Path(tmp))._fetch_remote_models(
                        AIProviderConfig(
                            name="TP",
                            env_key="TF3",
                            base_url="https://api.test3.com/v1",
                            signup_url="https://test.com",
                            is_free=True,
                            notes="t",
                        ),
...
class TestConfigWizardTestProvider:
    ...
    def test_test_provider_success(self, mock_console):
        ...
        with _patch_requests() as mock_req:
            ...
            with tempfile.TemporaryDirectory() as tmp:
                assert ConfigWizard(config_dir=Path(tmp))._test_provider(
                    AIProviderConfig(
                        name="TP",
                        env_key="TT_API_KEY",
                        base_url="https://api.test.com/v1",
...
class TestConfigWizardConfigureProvider:
    ...
    def test_configure_provider_skip(self, mock_console):
        ...
        mock_console.input.return_value = "s"
        with tempfile.TemporaryDirectory() as tmp:
            ConfigWizard(config_dir=Path(tmp))._configure_provider(
                AIProviderConfig(
                    name="TP",
                    env_key="TCP",
                    base_url="https://api.test.com/v1",
