...
class TestProtocolAnalyzer:
    ...
    def test_protocol_packet(self):
        ...
        p = ProtocolPacket(timestamp=0.0, src_addr=("127.0.0.1", 1883),
                          dst_addr=("127.0.0.1", 54321), protocol=ProtocolType.MQTT,
                          raw_data=b"\x10\x0c")
    ...
    def test_protocol_finding(self):
        ...
        f = ProtocolFinding(finding_id="f1", protocol=ProtocolType.MQTT,
                           finding_type="weak_auth", severity="high",
                           confidence=0.8, description="desc")
    ...
    def test_format_report(self):
        ...
        r = format_protocol_report({"target": "x", "findings": [{"title": "X", "severity": "high"}]})
...
class TestTargetedAttacks:
    def test_confirmed_finding(self):
        ...
        f = ConfirmedFinding(title="SQLi", severity="Critical", category="injection",
                             endpoint_url="http://t.com", method="POST", evidence="err")
        assert f.severity == "Critical"
        assert f.confidence == 1.0

    def test_defaults(self):
        from tools.targeted_attacks import ConfirmedFinding
        f = ConfirmedFinding(title="X", severity="High", category="xss",
                             endpoint_url="http://x", method="GET", evidence="e")
        assert f.payload == ""

    def test_full_params(self):
        from tools.targeted_attacks import ConfirmedFinding
        f = ConfirmedFinding(title="IDOR", severity="Medium", category="idor",
                             endpoint_url="http://api/v1/user/1", method="GET",
                             evidence="200 OK", response_snippet="data",
                             status_code=200, confidence=0.9, detector="idor")
...
class TestSmartScanner:
    def test_scan_phase_config(self):
        ...
        c = ScanPhaseConfig(name="recon", description="Recon phase",
...
class TestWAFEvasion:
    ...
    def test_waf_test_result(self):
        ...
        r = WAFTestResult(payload="test", techniques=["enc"], blocked=False,
                         status_code=200, response_snippet="ok",
                         waf_detected=None, confidence=0.9)
...
class TestSASTEngine:
    def test_code_vulnerability(self):
        ...
        v = CodeVulnerability(vuln_id="v1", file_path="test.py", line_number=10,
                             column=0, vuln_type="sqli", severity="high",
                             confidence=0.9, description="SQL injection",
                             code_snippet='os.system()', remediation="Use parameterized queries")
...
class TestPDFReport:
    def test_finding(self):
        ...
        f = Finding(title="XSS", severity="High", cvss_score=7.5,
                    description="Reflected XSS", impact="Session hijack",
                    evidence="alert(1)", remediation="Encode output")
        assert f.severity == "High"
...
class TestWordlistManager:
    def test_path_suggestion(self):
        ...
        p = PathSuggestion(path="/admin", confidence=0.9, reasoning="common",
                          estimated_severity="high", bounty_potential="high", source="common")
...
class TestWelcomeWizard:
    def test_setup_config(self):
        ...
        c = SetupConfig(ai_provider="openai", ai_model="gpt-4", default_mode="scan",
                       rate_limit=10, theme="dark", auto_update=True, telemetry=False)
...
class TestSOCAnalyzer:
    def test_alert(self):
        ...
        a = Alert(alert_id="a1", timestamp="2024-01-01", source="auth",
                  alert_type="brute_force", severity="high", confidence=0.9)
        assert a.severity == "high"

    def test_triage_result(self):
        from tools.soc_analyzer import Alert, TriageResult
        a = Alert(alert_id="a1", timestamp="t", source="s", alert_type="type",
                  severity="high", confidence=0.9)
        r = TriageResult(alert=a, priority_score=0.9, category="brute_force",
                        recommended_action="block", related_alerts=[])
        assert r.recommended_action == "block"

    def test_detection_rule(self):
        from tools.soc_analyzer import DetectionRule
        rule = DetectionRule(title="brute_force", logsource={"category": "auth"},
                            detection={"condition": "count > 10"}, tags=["attack"],
                            level="high", description="Brute force detection")
...
class TestWorkflowFuzzer:
    ...
    def test_plan(self):
        ...
        s = WorkflowStep(name="nav", method="GET", path="/login")
        p = WorkflowPlan(plan_id="p1", title="Test", description="desc",
                        steps=[s], risk="low")
        assert p.plan_id == "p1"
...
class TestObjectIDPermuter:
    def test_case(self):
        ...
        p = PermutationCase(endpoint_template="/api/user/{id}", placeholder="{id}",
                           value_a="1", value_b="2", description="ID comparison")
        assert p.value_a == "1"

    def test_result(self):
        from tools.object_id_permuter import PermutationResult
        r = PermutationResult(url_a="http://a/1", url_b="http://a/2",
                             status_a=200, status_b=403, len_a=100, len_b=50,
                             signal="idor", notes="different response")
...
class TestMultiAgent:
    def test_import(self):
        ...
        classes = [c for c in dir(ma) if not c.startswith('_') and isinstance(getattr(ma, c, None), type)]
...
class TestMiscModules:
    def test_vuln_hunter_core(self):
        ...
        classes = [c for c in dir(vhc) if not c.startswith('_') and isinstance(getattr(vhc, c, None), type)]
        assert len(classes) > 0
