    ...
    def get_filtered_sorted(self) -> List[Finding]:
        ...
        def sort_key(f: Finding):
            if self.sort_by == "severity":
                ...
                return (SEVERITY_CONFIG.get(f.severity.lower(), {}).get("priority", 99), f.timestamp)
            elif self.sort_by == "date":
                return (f.timestamp, SEVERITY_CONFIG.get(f.severity.lower(), {}).get("priority", 99))
        ...
        if not filtered:
            ...
            for i, finding in enumerate(filtered[:self.max_display]):
                ...
                if i == self.selected_index:
                    ...
                    title_text.append("  ")
                title_text.append(finding.title, style=f"bold {text_color}" if i == self.selected_index else text_color)
