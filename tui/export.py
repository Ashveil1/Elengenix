...
def export_to_html(
    ...
    Args:
        ...
        theme_name: Theme key from tui/themes.py (drives accent color).
    ...
    try:
        ...
        _theme = _get_theme(theme_name)
        _accent = _theme.get("primary", "#ff2222")
        _bg = _theme.get("bg_panel", "#0d0d0d")
        _card = _theme.get("bg_card", "#1a1a2e")
    except Exception:
        _accent, _bg, _card = "#ff2222", "#0d0d0d", "#1a1a2e"
...
"""
    ...
    html_content += f"""
        ...
        <div class="filters" style="margin-bottom:20px;">
            ...
            <select id="sev-filter" onchange="filterSev(this.value)"
                style="background:{_card};color:#fff;border:1px solid #333;border-radius:6px;padding:4px 8px;">
        ...
        <script>
        function filterSev(v) {{
            document.querySelectorAll('.finding').forEach(el => {{
                el.style.display = (v === 'all' || el.dataset.severity === v) ? '' : 'none';
            }});
        }}
        </script>

        <div class="footer">
            Generated by Elengenix Security Framework (theme: {html_module.escape(theme_name)})
        </div>
    </div>
</body>
</html>
"""
    # Apply unified accent/background (keep white/black/red identity by default).
    html_content = (
        html_content.replace("#ff2222", _accent)
        .replace("#0d0d0d", _bg)
        .replace("#1a1a2e", _card)
    )
...
def export_to_pdf(
    ...
    """Export dashboard data to a real PDF file.

    Uses weasyprint when installed (pip install elengenix[pdf]); otherwise
    falls back to export_to_svg() and returns the .svg path so callers never
    crash on minimal installs.
    """
    try:
        from weasyprint import HTML  # type: ignore[import-not-found]

        html_path = output_path + ".tmp.html"
        export_to_html(dashboard_data, html_path, title=title, theme_name=theme_name)
        ...
        try:
            ...
            pass
        logger.info(f"Exported dashboard to PDF: {pdf_path}")
        return pdf_path
    except ImportError:
        logger.warning("weasyprint not installed; falling back to SVG export")
        return export_to_svg(dashboard_data, output_path, title=title)
    except Exception as exc:
        logger.warning(f"PDF export failed ({exc}); falling back to SVG")
        return export_to_svg(dashboard_data, output_path, title=title)
