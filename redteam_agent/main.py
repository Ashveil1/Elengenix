...
from redteam_agent.prompts import STRATEGIST_PROMPT, SPECIALIST_PROMPT
...
def run_specialist_cycle(llm: LLMClient, executor: ShellExecutor, state: MissionState) -> bool:
    ...
    if action == "run_command":
        ...
        status_color = "\033[92m[OK]\033[0m" if result["success"] else "\033[91m[FAIL]\033[0m" # Green OK, Red FAIL
...
def get_default_base_url(provider_id: str) -> str:
    if provider_id == "gemini": return "https://generativelanguage.googleapis.com/v1beta/openai/v1/chat/completions"
    elif provider_id == "openai": return "https://api.openai.com/v1/chat/completions"
    elif provider_id == "nvidia": return "https://integrate.api.nvidia.com/v1/chat/completions"
    elif provider_id == "anthropic": return "https://api.anthropic.com/v1/messages"
    elif provider_id == "ollama": return "http://localhost:11434/v1/chat/completions"
    return ""
...
def fetch_models(provider_id: str, base_url: str, api_key: str) -> list[str]:
    ...
    try:
        if provider_id == "ollama":
            ...
            models = ["claude-3-7-sonnet-latest", "claude-3-5-sonnet-latest", "claude-3-haiku-20240307", "claude-3-opus-20240229"]
        else:
...
def setup_slot(slot: int, current_provider: str):
    ...
    if provider_id != "ollama":
        ...
        if api_key:
            env_key_name = f"{provider_id.upper()}_API_KEY" if provider_id != "custom" else f"AI{slot}_CUSTOM_API_KEY"
            save_to_env(env_key_name, api_key)
            
    # Resolve URL for fetching models
    import os
    actual_base_url = base_url or (os.getenv(f"AI{slot}_CUSTOM_API_BASE") if provider_id == "custom" else get_default_base_url(provider_id))
        
    # Attempt to resolve api key if blank so we can fetch models
    resolved_key = api_key
    if not resolved_key:
        if provider_id == "gemini": resolved_key = os.getenv("GEMINI_API_KEY")
        elif provider_id == "openai": resolved_key = os.getenv("OPENAI_API_KEY")
        elif provider_id == "nvidia": resolved_key = os.getenv("NVIDIA_API_KEY")
        elif provider_id == "anthropic": resolved_key = os.getenv("ANTHROPIC_API_KEY")
        elif provider_id == "custom": resolved_key = os.getenv(f"AI{slot}_CUSTOM_API_KEY")
    ...
    if available_models:
        ...
        m_choice = input(f"\nSelect Model number (or type custom name) [Enter to keep current/default]: ").strip()
        if m_choice.isdigit():
            idx = int(m_choice) - 1
