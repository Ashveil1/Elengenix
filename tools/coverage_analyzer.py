...
class CoverageAnalyzer:
    ...
    def record_test(
        ...
        cur.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                record.url, record.method, record.tool, record.injection_point,
                record.payload[:500], record.status, record.response_size,
                int(record.is_interesting), record.timestamp, record.notes,
            ),
        )
        # Update last_tested
        cur.execute(
            "UPDATE endpoints SET last_tested=? WHERE endpoint_key=?",
            (record.timestamp, f"{record.method} {urlparse(url).scheme}://{urlparse(url).netloc}{urlparse(url).path or '/'}"),
        )
        self._conn.commit()
        return record
    ...
    def get_untested_endpoints(self) -> List[EndpointRecord]:
        ...
        return [
            EndpointRecord(
                ...
                params=json.loads(r["params_json"] or "[]"),
                headers=json.loads(r["headers_json"] or "[]"),
                first_seen=r["first_seen"], source=r["source"],
            )
            for r in rows
        ]
    ...
    def get_endpoint_coverage(self, url: str) -> Dict[str, Any]:
        ...
        cur = self._conn.cursor()
        endpoint_key = f"GET {urlparse(url).scheme}://{urlparse(url).netloc}{urlparse(url).path or '/'}"
        ep_row = cur.execute("SELECT * FROM endpoints WHERE endpoint_key=?", (endpoint_key,)).fetchone()
        if not ep_row:
        ...
        return {
            ...
            "coverage_pct": (sum(1 for v in param_coverage.values() if v > 0) / max(len(params), 1)) * 100,
    ...
    def suggest_next_targets(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        high_value = ["id", "user_id", "uid", "file", "path", "q", "url", "redirect", "next", "return"]
