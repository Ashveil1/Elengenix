...
class LogicFlawEngine:
    ...
    def _test_price_manipulation(
        ...
        price_params = [k for k in params.keys() if any(p in k.lower() for p in ["price", "amount", "cost", "total"])]
    ...
    def _test_quantity_overflow(
        ...
        qty_params = [k for k in params.keys() if any(p in k.lower() for p in ["quantity", "qty", "count", "amount"])]
