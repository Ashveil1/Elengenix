...
def run_omni_scan(
    ...
    console.print(Panel(
        f"[bold red]⚔️  ELENGENIX FULL-SCALE MISSION v3.0[/bold red]\n"
        f"🎯 Target: [cyan]{safe_target}[/cyan]\n"
        f"🛠️  Tools Available: {tool_count}/{len(available_tools)}\n"
        f"⚡ Rate Limit: {rate_limit} concurrent | CVSS: {'✓' if enable_cvss else '✗'}",
        border_style="red",
    ))
    
    if tool_count == 0:
        console.print("[yellow]⚠️  Warning: No tools available. Install required binaries.[/yellow]")
        console.print("[dim]Run: elengenix doctor[/dim]")
    
    send_telegram_notification(f"⚔️ Full scan v3.0 initiated: `{safe_target}`")
...
def _print_findings_table(findings: List[Dict[str, Any]]) -> None:
    ...
    if not findings:
        console.print("[dim]ℹ️  No findings detected.[/dim]")
        return
...
def list_available_tools() -> None:
    ...
    table = Table(title="🛠️  Registered Security Tools")
