...
class SchemaDiff:
    ...
    def has_changes(self) -> bool:
        return len(self.added_endpoints) > 0 or len(self.removed_endpoints) > 0 or len(self.modified_endpoints) > 0
...
class APISchemaDiffer:
    ...
    def _detect_schema_type(self, schema: Dict[str, Any]) -> str:
        ...
        if "__schema" in schema:
            return "graphql"
        if "types" in schema and "queryType" in schema.get("types", [{}])[0] if schema.get("types") else False:
            return "graphql"
        return "unknown"
    ...
    def _compare_openapi(
        ...
        for endpoint in endpoints2 - endpoints1:
            ...
            result.added_endpoints.append(EndpointDiff(
                path=path,
                method=method,
                change_type="added",
        ...
        for endpoint in endpoints1 - endpoints2:
            path, method = endpoint
            result.removed_endpoints.append(EndpointDiff(
                path=path,
                method=method,
                change_type="removed",
        ...
        for endpoint in endpoints1 & endpoints2:
            ...
            if old_endpoint != new_endpoint:
                ...
                if changes:
                    result.modified_endpoints.append(EndpointDiff(
                        path=path,
                        method=method,
