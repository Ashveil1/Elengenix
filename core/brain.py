...
"""
    ...
    def process_query(
        ...
        Args:
            ...
            use_smart_scan: Use SmartOrchestrator engine.
            use_new_pipeline: Use the new modular pipeline (Wave 1 refactoring).
        """
        # New pipeline path
        if use_new_pipeline:
            return self._process_query_new(user_input, target, callback)
...
"purpose": "...", "question": "..."}}"""
    ...
    # ── New Pipeline (Wave 1 Refactoring) ─────────────────────────────

    def _process_query_new(
        self,
        user_input: str,
        target: str = "",
        callback: Optional[Callable] = None,
    ) -> str:
        """Process a scan using the new modular pipeline.

        This method uses the extracted modules (ScanContext, PromptBuilder,
        DecisionEngine, PostProcessor, ScanLoop) instead of inline code.
        It's an alternative to the legacy process_query() code path.
        ...
        """
        # Lazy imports to avoid breaking existing system
        from agents.scan_context import ScanContext
        ...
        mission_state.upsert_node(
            GraphNode(
                node_id=ctx.target,
                node_type="target",
                props={"target": ctx.target},
            )
        )
        ctx.mission_state = mission_state

        # 3. Initialize meta-cognitive components
        from tools.vuln_hunter_core import (
            BeliefState,
            CoverageMap,
            NegativeResultStore,
        ...
        ctx.belief_state = BeliefState(ctx.mission_key)
        ctx.coverage_map = CoverageMap(ctx.mission_key, ctx.target)
        ctx.negative_results = NegativeResultStore(ctx.mission_key)
        ctx.verification_pipeline = VerificationPipeline(agent=self)
        ctx.reflect_engine = ReflectEngine(adapt_after=5, max_steps_without_findings=8)

        # 4. Initialize VulnFinder
        vf_module = _get_vuln_finder()
        vuln_finder = vf_module.VulnFinder(
            target=ctx.target,
            max_steps=self.max_steps,
        ...
        if self.enable_planning and target:
            fingerprint = self._fingerprint_target_for_planning(target)
            try:
                ctx.attack_tree = self.planner.generate_attack_tree(
                    target,
                    objective=user_input,
                    fingerprint=fingerprint,
                )
        ...
        post_processor = PostExecutionProcessor(
            ...
            vuln_reasoning=self.vuln_reasoning,
            planner=self.planner,
            vuln_finder=vuln_finder,
            callback=callback,
        )

        # 7. Create executor wrapper
        def executor(action_data, ctx):
            return self._execute_with_governance(action_data, ctx, target, callback)

        # 8. Create and run scan loop
        scan_loop = ScanLoop(
        ...
        # 10. Remember mission completion
        try:
    ...
    def _execute_with_governance(
        ...
        """Execute an action with governance gate.

        This wraps the governance check and tool execution into a callable
        that ScanLoop can use as its executor.

        Args:
            action_data: The action to execute.
        ...
        # Handle submit_findings (already handled by ScanLoop)
        if action == "submit_findings":
            return False, None, "handled_by_scan_loop"
        ...
        if not gate_decision.allowed:
            if gate_decision.decision == "needs_approval":
                try:
                    ...
                    approved = confirm(
                        f"Approve high-risk action?\n\nTool: {tool_name}\n"
                        f"Purpose: {action_data.get('purpose', '')}",
                        default=False,
                    )
                except ImportError:
                    approved = False
                ...
                if not approved:
                    return False, None, f"Governance: rejected (risk={gate_decision.risk_level})"
        ...
        return False, None, "No tool or command specified"
